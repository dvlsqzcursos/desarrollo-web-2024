<?php
    ...
    class prestamoControlador extends prestamoModelo{
		...
		public function buscar_cliente_prestamo_controlador(){
			...
			if($cliente==""){
				return '<div class="alert alert-warning" role="alert">
					<p class="text-center mb-0">
						...
						Debes de introducir el DPI, Nombre, Apellido o Teléfono del cliente
					</p>
				</div>';
				exit();
			}
			...
			$datos_cliente=mainModel::ejecutar_consulta_simple("SELECT * FROM cliente WHERE cliente_dpi LIKE '%$cliente%' OR cliente_nombre LIKE '%$cliente%' OR cliente_apellido LIKE '%$cliente%' OR cliente_telefono LIKE '%$cliente%' ORDER BY cliente_nombre ASC");
			...
			if($datos_cliente->rowCount()>=1){
				...
				foreach($datos_cliente as $rows){
					...
					<tr class="text-center">
                        <td>'.$rows['cliente_nombre'].' '.$rows['cliente_apellido'].' ('.$rows['cliente_dpi'].')</td>
                        <td>
		...
		public function agregar_prestamo_controlador(){
			...
			if(empty($_SESSION['datos_cliente'])){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No has agregado el cliente para realizar el préstamo o reservación.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
